...
function findHandType(cards: Card[]): Type {
    ...
    if (jokerCount === 5) return Type.FiveOfAKind // edge-case where jokers have no other card to copy
    const [highestCountCard, highestCount] = Object.entries(cardCounts).reduce(
        (highestCardCount, cardCount) => (cardCount[1] > highestCardCount[1] ? cardCount : highestCardCount),
        ['', 0]
    )
    cardCounts[highestCountCard] += jokerCount ?? 0
...
function solver(input: string[]): number {
    ...
    const answer = hands.reduce((ans, hand, i) => {
        ...
        output.push(`${`${rank}`.padStart(4, ' ')} ${hand.cards.join('')} ${Type[hand.type]}`)
        return ans + hand.bid * rank
    }, 0)

    writeOutput({ day: 7, part: 2, output })
    return answer
}
