...
export async function solveCalendar(props: {
    ...
    solver: (input: string[]) => number | Promise<number>
    ...
    if (props.sampleAnswer !== undefined) {
        ...
        const sampleAnswer = await props.solver(sampleInput)
    ...
    const answer = await props.solver(input)
    console.log(`\n${answer} ⭐️\n`)
}
