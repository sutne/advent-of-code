...
function solver(input: string[]): number {
    ...
    const answer = sequences.reduce((sum, sequence) => {
        ...
        const prediction = subsequences.reverse().reduce((prediction, diff) => diff.at(-1) + prediction, 0)

        output.push(`${sequence.join(' ')} -> ${prediction}`)
        return sum + prediction
    }, 0)

    writeOutput({ day: 9, part: 1, output })
    return answer
}
